import { Settings, Bitcoin, ArrowDownUp, SearchIcon, AlignJustify, TrendingUp, AlertTriangle, ArrowUp, ChevronLeft, ChevronRight, ArrowDown, RotateCcw, RefreshCw, Eye, EyeOff, Copy, Code, Clock, CheckCircle, DollarSign } from 'lucide-react';
...
export function WalletTab() {
  ...
  const [activityPeriod] = useState<'today' | 'week' | 'month'>('week');
  ...
  const filteredTransactions = transactions.filter(tx => {
    ...
    const matchesCategory =
      categoryFilter === 'all' ||
      (categoryFilter === 'sent' && (tx.type === 'sent' || tx.type === 'self')) ||
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#24292D] to-[#1E2328] p-2 sm:p-4 md:p-6">
      <div className="container mx-auto px-0 sm:px-2 md:px-4 py-4 sm:py-6 max-w-7xl">
        ...
        {/* Balance Card */}
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-600 via-teal-600 to-cyan-700 p-4 sm:p-6 md:p-8 mb-6 sm:mb-8 shadow-2xl">
            ...
            <div className="flex flex-wrap justify-center gap-3 sm:gap-4 md:gap-6 mt-3 sm:mt-4">
              {[
                ...
                <div key={item.label} className="flex flex-col items-center group">
                  <button
                    className="cursor-pointer flex items-center justify-center w-14 h-14 md:w-16 md:h-16 bg-white/10 backdrop-blur-sm rounded-2xl text-white transition-all duration-300 hover:bg-white/20 hover:scale-110 hover:shadow-lg border border-white/20 disabled:opacity-50 disabled:cursor-not-allowed"
                    onClick={item.action}
        ...
        <div className="flex justify-center mb-6 sm:mb-8">
          <div className="inline-flex bg-[#2F363E]/60 backdrop-blur-xl rounded-2xl p-1 border border-white/10 shadow-xl ">
            <button
              onClick={() => setViewMode('transactions')}
              className={`cursor-pointer px-4 md:px-6 py-2 md:py-3 rounded-xl font-medium transition-all duration-300 text-sm ${viewMode === 'transactions'
            ...
            <button
              ...
              className={`cursor-pointer px-4 md:px-6 py-2 md:py-3 rounded-xl font-medium transition-all duration-300 text-sm ${viewMode === 'analysis'
                ? 'bg-gradient-to-r from-teal-500 to-teal-600 text-white shadow-lg'
                : 'text-gray-300 hover:text-white hover:bg-white/5'
                }`}
            >
        ...
        {viewMode === 'transactions' && (
          <>
            ...
            <div className="mb-4 sm:mb-3">
              <div className="p-3 sm:p-3 md:p-3  bg-[#2F363E]/60 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl relative z-10">
                <div className="flex flex-col lg:flex-row gap-3 sm:gap-4 items-center">
                  ...
                  <div className="flex sm:flex-row gap-2 sm:gap-3">
                    {/* Status Listbox com ícone */}
                    <Listbox value={statusFilter} onChange={setStatusFilter}>
                      <div className="relative min-w-[160px]">
                        <Listbox.Button className="cursor-pointer w-full px-4 py-3 bg-[#24292D]/80 backdrop-blur-sm border border-white/10 rounded-xl text-white placeholder-gray-400 transition-all text-sm text-left whitespace-nowrap hover:bg-[#2d3338] truncate flex items-center gap-2">
                          {/* Ícone do filtro de status */}
                          {statusFilter === 'all' && <AlignJustify size={16} className="text-gray-400" />}
                          {statusFilter === 'confirmed' && <CheckCircle size={16} className="text-green-400" />}
                          {statusFilter === 'pending' && <Clock size={16} className="text-yellow-400" />}
                          {statusFilter === 'error' && <AlertTriangle size={16} className="text-red-400" />}
                          <span>
                        ...
                        <Listbox.Options className="text-white absolute w-full bg-[#24292D] border border-white/10 rounded-xl shadow-lg z-20 mt-1">
                          {statusOptions.map(opt => (
                            <Listbox.Option key={opt.value} value={opt.value} className="px-4 py-2 hover:bg-[#2d3338] cursor-pointer ui-active:bg-[#2d3338] ui-selected:font-semibold text-sm flex items-center gap-2">
                              {opt.value === 'all' && <AlignJustify size={16} className="text-gray-400" />}
                              {opt.value === 'confirmed' && <CheckCircle size={16} className="text-green-400" />}
                              {opt.value === 'pending' && <Clock size={16} className="text-yellow-400" />}
                              {opt.value === 'error' && <AlertTriangle size={16} className="text-red-400" />}
                              {opt.label}
                            </Listbox.Option>
                          ))}
                        </Listbox.Options>
                    ...
                    {/* Categoria Listbox com ícone */}
                    <Listbox value={categoryFilter} onChange={setCategoryFilter}>
                      <div className="relative min-w-[160px]">
                        <Listbox.Button className="cursor-pointer w-full px-4 py-3 bg-[#24292D]/80 backdrop-blur-sm border border-white/10 rounded-xl text-white placeholder-gray-400 transition-all text-sm text-left whitespace-nowrap hover:bg-[#2d3338] truncate flex items-center gap-2">
                          {/* Ícone do filtro de categoria */}
                          {categoryFilter === 'all' && <AlignJustify size={16} className="text-gray-400" />}
                          {categoryFilter === 'sent' && <ArrowUp size={16} className="text-red-400" />}
                          {categoryFilter === 'received' && <ArrowDown size={16} className="text-green-400" />}
                          <span>
                        ...
                        <Listbox.Options className="text-white absolute w-full bg-[#24292D] border border-white/10 rounded-xl shadow-lg z-20 mt-1">
                          {categoryOptions.map(opt => (
                            <Listbox.Option key={opt.value} value={opt.value} className="px-4 py-2 hover:bg-[#2d3338] cursor-pointer ui-active:bg-[#2d3338] ui-selected:font-semibold text-sm flex items-center gap-2">
                              {opt.value === 'all' && <AlignJustify size={16} className="text-gray-400" />}
                              {opt.value === 'sent' && <ArrowUp size={16} className="text-red-400" />}
                              {opt.value === 'received' && <ArrowDown size={16} className="text-green-400" />}
                              {opt.label}
                            </Listbox.Option>
                          ))}
                        </Listbox.Options>
            ...
            {/* Transactions Table */}
            <div
              className="bg-[#2F363E]/60 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl overflow-x-auto">
              {loading ? (
                    ...
                    <thead className="bg-[#24292D]/80 backdrop-blur-sm border-b border-white/10">
                      <tr className="text-xs">
                        <th className="px-4 py-3 text-left font-semibold text-gray-300 uppercase tracking-wider">
                          <ArrowDownUp size={16} className="inline mr-1 text-gray-300" /> Transação</th>
                        <th className="px-4 py-3 text-left font-semibold text-gray-300 uppercase tracking-wider hidden md:table-cell">
                          <Clock size={16} className="inline mr-1 text-gray-300" /> Data</th>
                        <th className="px-4 py-3 text-left font-semibold text-gray-300 uppercase tracking-wider">
                          <Bitcoin size={16} className="inline mr-1 text-gray-300" />Quantidade</th>
                        <th className="px-4 py-3 text-left font-semibold text-gray-300 uppercase tracking-wider">
                          <CheckCircle size={16} className="inline mr-1 text-gray-300" /> Status</th>
                        <th className="px-4 py-3 text-left font-semibold text-gray-300 uppercase tracking-wider">
                          <Settings size={16} className="inline mr-1 text-gray-300" /> Ação</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredTransactions.map((tx) => (
                        ...
                        >
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <div
                                className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${tx.type === 'received'
                          ...
                          <td className="px-4 py-3 text-gray-200 text-xs hidden md:table-cell">
                           <Clock size={16} className="inline mr-1 text-gray-300" /> {formatDate(tx.timestamp)}
                          </td>
                          <td className="px-4 py-3">
                            <div className={`text-xs md:text-sm font-medium ${tx.type === 'received' ? 'text-green-400' : (tx.type === 'sent' || tx.type === 'self') ? 'text-red-400' : 'text-white'}`}>
                              <Bitcoin size={16} className="inline mr-1 text-yellow-500" />{tx.type === 'received' ? '+' : (tx.type === 'sent' || tx.type === 'self') ? '-' : ''}{tx.amountBCH.toFixed(8)} BCH
                            </div>
                            <div className="text-gray-300 text-xs">
                              <DollarSign size={16} className="inline mr-1 text-green-300" />{formatCurrency(tx.amountBRL)}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ${tx.status === 'confirmed'
                            ...
                            >
                              <CheckCircle size={16} className="inline mr-1 text-green-300" />{tx.status === 'confirmed' ? `Confirmado (${tx.confirmations > 99 ? "99+" : tx.confirmations})` : tx.status === 'pending' ? `Pendente (${tx.confirmations})` : 'Erro'}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <a
            ...
            {!loading && !error && filteredTransactions.length > 0 && (
              <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row items-center justify-between px-2 sm:px-4 py-2 sm:py-3 bg-[#2F363E]/60 backdrop-blur-xl rounded-xl border border-white/10 shadow-xl gap-2">
                <div>
        ...
        {viewMode === 'analysis' && (
          <div className="bg-[#2F363E]/60 backdrop-blur-xl rounded-2xl border border-white/10 p-3 sm:p-4 md:p-6 shadow-xl">
            ...
            <div className="h-52 sm:h-64 md:h-80 mb-4 sm:mb-6">
        ...
        {sendModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-2 sm:p-4">
            <div className="bg-[#2F363E] rounded-2xl w-full max-w-xs sm:max-w-md shadow-2xl relative border border-[#3A414A]/70">
              <button
              ...
              <form onSubmit={handleSendSubmit} className="p-6 space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <input
                      type="text"
                      placeholder="Digite o endereço BCH (Ex: bitcoincash:...)"
                      ...
                      disabled={isSending}
                    />
                    <button type="button" title="Scan QR (Em breve)" className="absolute right-3 top-1/2 transform -translate-y-1/2 text-teal-400 hover:text-teal-300 disabled:opacity-50 disabled:cursor-not-allowed" disabled> <Code size={20} /> </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  {/* Botão para usar saldo disponível aproximado */}
                  <button
                    type="button"
                    onClick={() => {
                      const available = balance.availableBCH;
                      const usableAmount = Math.max(0, available - (estimatedFeeClientSide * 1.05));
                      if (usableAmount > 0) {
                    ...
                    className="text-xs cursor-pointer text-teal-400 hover:text-teal-300 disabled:opacity-50 disabled:cursor-not-allowed mt-1"
                  >
                    Usar saldo disponível (aprox.)
                  </button>
                </div>

                {/* Select de taxa de rede */}
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1.5">Taxa de Rede</label>
                  <select
                    className="w-full px-4 py-3 rounded-xl cursor-pointer bg-[#24292D] border border-[#3A414A] text-white focus:outline-none focus:ring-2 focus:ring-teal-500"
                    value={sendForm.fee}
                ...
                {/* Exibição da taxa estimada e total */}
                ...
                <button
                  type="submit"
                  className={`w-full cursor-pointer bg-teal-600 text-white rounded-xl py-3 font-semibold hover:bg-teal-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center ${isSending ? 'animate-pulse' : ''}`}
                  disabled={isSending || !sendForm.address || !sendForm.amountBCH || parseFloat(sendForm.amountBCH) <= 0}
                >
                  {isSending ? <><Clock className="animate-spin h-5 w-5 mr-2" /> Enviando...</> : <><ArrowUp className="h-5 w-5 mr-1" /> Enviar Transação</>}
                </button>
              </form>
            </div>
          </div>
        ...
        {receiveModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-2 sm:p-4">
            <div className="bg-[#2F363E] rounded-2xl w-full max-w-xs sm:max-w-md shadow-2xl relative border border-[#3A414A]/70">
              <button
                className="absolute cursor-pointer top-4 right-5 text-gray-400 hover:text-white text-2xl transition-colors"
              ...
              {/* QrCode com endereço BCH da carteira do usuário */}
              <div className="p-6 text-center space-y-6">
                ...
                {/* Link copiavel do endereço BCH do usuário */}
                <div>
                  ...
                  <div className="bg-[#24292D]/70 rounded-xl p-3 border border-[#3A414A]/70">
                    ...
                    <button
                      className="w-full cursor-pointer bg-teal-600 text-white rounded-lg py-2.5 font-medium hover:bg-teal-700 transition-colors flex items-center justify-center gap-2"
                      onClick={copyToClipboard}
                      disabled={!walletAddress || isCopied}
                    >
                      {isCopied ? <><CheckCircle size={18} />Copiado!</> : <><Copy size={18} />Copiar Endereço</>}
                    </button>
                  </div>
                </div>
                <button
                  className="w-full cursor-pointer bg-[#3A414A] text-white rounded-xl py-3 font-medium hover:bg-[#4A5568] transition-colors border border-[#4A5568]/70"
                  onClick={() => {
                    if (navigator.share && walletAddress) {
                      navigator.share({ title: 'Meu Endereço BCH', text: walletAddress });
        ...
        {/* Modal de sucesso ao enviar BCH*/}
        {showSuccessModal && lastSent && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-2 sm:p-4">
            <div className="bg-[#2F363E] rounded-2xl w-full max-w-xs sm:max-w-md shadow-2xl relative border border-[#3A414A]/70">
              <button
                className="cursor-pointer absolute top-4 right-5 text-gray-400 hover:text-white text-2xl transition-colors"
              ...
              <div className="p-6 md:p-8 text-center">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-teal-600/20 border-2 border-teal-500/50 rounded-full flex items-center justify-center mx-auto mb-5">
                  <FiCheckCircle className="w-8 h-8 md:w-10 md:h-10 text-teal-400" />
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button
                    className="flex-1 cursor-pointer bg-teal-600 text-white rounded-xl py-2.5 md:py-3 font-medium hover:bg-teal-700 transition-colors"
                    onClick={() => setShowSuccessModal(false)}>
                    Continuar
                  </button>
                  <button
                    ...
                    className="flex-1 bg-[#3A414A] cursor-pointer text-white rounded-xl py-2.5 md:py-3 font-medium hover:bg-[#4A5568] transition-colors border border-[#4A5568]/70"
